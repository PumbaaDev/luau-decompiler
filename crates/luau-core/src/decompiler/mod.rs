...
pub enum RegisterHint {
    ...
    Named(String),
    /// NOT A NAME - a barrier marking a write that produces a value UNRELATED
    /// to whatever the register held before.
    ///
    /// Hints are append-only, so a name recorded for a previous occupant of a
    /// register outlives it. Scoping every hint to the most recent write was
    /// measured and REJECTED: it deletes deliberate inheritance, where
    /// `GETTABLEKS .Field` names the function and the following `CALL`
    /// overwrites the register with the result that should keep that name.
    ///
    /// The discriminator is derivation, not recency. Only a write whose value
    /// cannot derive from the register's previous contents - an import, an
    /// upvalue, a constant, a fresh table or closure - ends the name's life.
    /// A corpus case needs exactly this: R13 holds a `math.min` result, then
    /// `GETIMPORT R13` reuses the register for something unrelated.
    FreshWrite,
}
...
pub struct DecompileContext<'a> {
    ...
    /// Upvalue slots whose CAPTURE source register genuinely held a Roblox
    /// instance global (`game` / `workspace` / `script`) at capture time.
    /// Naming such an upvalue after the global is EXACT — the captured value IS
    /// that global — so `upval_name` lets it through the `is_stdlib_shadow_name`
    /// guard that otherwise (correctly) rejects a MISINFERRED global name.
    /// Keyed by child proto index -> set of upvalue slot indices.
    pub capture_global_upvals: std::collections::HashMap<usize, std::collections::HashSet<u8>>,
...
impl<'a> DecompileContext<'a> {
    pub fn new(chunk: &'a Chunk) -> Self {
        Self {
            ...
            capture_global_upvals: std::collections::HashMap::new(),
    ...
    fn synthesize_name(&mut self, reg: u8, pc: usize) -> String {
        ...
        let prefix = match hint {
            // `select_hint` never returns the barrier; it is a lifetime marker,
            // not a name. Treat it as no hint at all.
            Some(RegisterHint::FreshWrite) => {
                // Unreachable in practice - `select_hint` never returns the
                // barrier. Mirror the no-hint arm rather than inventing a
                // third naming path.
                let p = format!("v{}", reg);
    ...
    pub fn upval_name(&self, proto: &Proto, proto_index: usize, idx: u8) -> String {
        ...
        if let Some(names) = self.inferred_upvalue_names.get(&proto_index) {
            if let Some(name) = names.get(idx as usize) {
                ...
                // Exception: a CAPTURE whose source register genuinely held a
                // Roblox instance global (game/workspace/script) is EXACT — the
                // upvalue's value IS that global — so it may keep the global name
                // even though `is_stdlib_shadow_name` lists those three. The
                // trusted set is populated ONLY from a real remapped CAPTURE
                // whose register held that exact global expression, never from a
                // usage-based guess, so this cannot resurrect a misinference.
                let trusted_global = matches!(name.as_str(), "game" | "workspace" | "script")
                    && self
                        .capture_global_upvals
                        .get(&proto_index)
                        .is_some_and(|s| s.contains(&idx));
                if !name.is_empty()
                    && is_valid_luau_identifier(name)
                    && (!is_stdlib_shadow_name(name) || trusted_global)
                {
                    return name.clone();
                }
            }
...
fn select_hint(hints: &[(usize, RegisterHint)], pc: usize) -> Option<RegisterHint> {
    fn pick(hs: impl Iterator<Item = (usize, RegisterHint)> + Clone) -> Option<RegisterHint> {
        ...
        for (_, h) in collected.iter().rev() {
            if !matches!(
                h,
                RegisterHint::Param(_) | RegisterHint::SelfParam | RegisterHint::FreshWrite
            ) {
                return Some(h.clone());
            }
        }
        collected
            .iter()
            .find(|(_, h)| !matches!(h, RegisterHint::FreshWrite))
            .map(|(_, h)| h.clone())
    }

    // A `FreshWrite` at or before this read ends the previous value's life, so
    // only hints from that point on describe what the register holds now.
    let barrier = hints
        .iter()
        .filter(|(hpc, h)| *hpc <= pc && matches!(h, RegisterHint::FreshWrite))
        .map(|(hpc, _)| *hpc)
        .max()
        .unwrap_or(0);
    let in_scope = hints
        .iter()
        .filter(|(hpc, _)| *hpc <= pc && *hpc >= barrier)
        .cloned();
...
pub fn analyze_register_usage(
    ...
    while pc < code.len() {
        ...
        // Record the barrier BEFORE this instruction's own hint, so the hint
        // it pushes at the same pc stays inside the new value's lifetime.
        if matches!(
            op,
            // NOT `GetImport`: a `require(Path.To.Module)` result legitimately
            // inherits its name FROM the import, and barriering it replaces
            // `local DynamicThumbstick2 = require(Thumbstick)` with
            // `local import3 = ..` - measured on a corpus module.
            ...
            hints.entry(a).or_default().push((pc, RegisterHint::FreshWrite));
        }
