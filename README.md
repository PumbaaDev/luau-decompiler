...
**Contents:** [Highlights](#highlights) · [Features](#features) · [Install](#install) · [Usage](#usage) · [Try it end-to-end](#try-it-end-to-end) · [Use it as a library](#use-it-as-a-library) · [Correctness](#correctness) · [Project layout](#project-layout) · [How it works](#how-it-works) · [Contributing](#contributing)
...
- **Embed it anywhere** — use `luau-core` as a Rust crate, or link the **C API** (`luau-ffi`, a `cdylib`/`staticlib` with a hand-written header) from C, C++, Python (ctypes/cffi), Go (cgo), or any language with a C FFI.
...
The engine is not CLI-only. The same decompiler is available as a Rust crate and through a C ABI, so you can embed it in your own tools instead of shelling out.

### Rust

`luau-core` is the whole engine. Add it as a path/git dependency and call it directly:

```rust
let bytecode: Vec<u8> = std::fs::read("script.luac")?;
let source = luau_core::decompile(&bytecode)?;      // readable Luau
let listing = luau_core::disassemble(&bytecode, true)?;  // with debug info
let info = luau_core::info(&bytecode)?;              // protos, strings, stack sizes
println!("{source}");
```

### C / C++ / any language with a C FFI

`luau-ffi` wraps the engine in a stable C ABI (`cdylib` + `staticlib`) with a hand-written header at `crates/luau-ffi/include/luau_decompiler.h`. It is panic-safe (no unwind crosses the boundary), guards null and oversized input, and hands ownership of returned strings to the caller.

```sh
cargo build --release -p luau-ffi
# produces target/release/{libluau_ffi.so | luau_ffi.dll | libluau_ffi.dylib} + a staticlib
```
...
char *src = NULL;
int rc = luau_decompile(bytecode, len, &src);   /* 0 == LUAU_OK */
if (rc == LUAU_OK) {
    puts(src);
    luau_string_free(src);                       /* caller frees */
} else {
    fprintf(stderr, "decompile failed: %s\n", luau_last_error());
}
```

The surface is small and documented in the header:

```c
int         luau_decompile(const uint8_t *bytecode, size_t len, char **out_source);
int         luau_disassemble(const uint8_t *bytecode, size_t len, int show_debug, char **out_text);
int         luau_info_json(const uint8_t *bytecode, size_t len, char **out_json);
int         luau_bytecode_version(const uint8_t *bytecode, size_t len); /* version byte, or -1 */
const char *luau_last_error(void);   /* thread-local; do not free */
void        luau_string_free(char *s);
const char *luau_version(void);      /* static; do not free */
```

A runnable C example is in `crates/luau-ffi/examples/decompile.c`, and per-platform link instructions are in `crates/luau-ffi/README.md`. Calling from Python (`ctypes`/`cffi`), Go (`cgo`), or Node (`ffi-napi`) is the same three-call pattern: decompile, use, free.
...
whole live Roblox game plus Roblox CoreScripts, the PlayerModule, and widely-used
community modules (topbarplus, Janitor, evaera-promise) — decompiled in one clean
batch run against a measured opcode map.
...
**On `bodies_dropped`.** On the shipped path — `decompile()` with the bundled
measured opcode map — `CameraModule` emits all of its bodies (31 of 32 protos,
0 findings). The drop reproduces only on the detector-only path (opcode-shuffled
input with no measured map), where a wrong permutation is recovered for this hard
39-distinct-opcode file — the shuffled-path ceiling described above, not a lifter bug.
...
| Crate | What it is |
|-------|------------|
| `luau-core` | The decompiler engine — parser, opcode mapping, AST, lifter, emitter. All the real logic lives here. |
| `luau-cli`  | The command-line front-end (`luau-decompiler`): decompile, disassemble, info, batch, watch, validate, compare. |
| `luau-compiler` | The companion Luau obfuscator/protector (`luau-protect`). |
| `luau-ffi` | The C ABI (`cdylib`/`staticlib`) + header, so the engine can be called from C, C++, Python, Go, and friends. |
...
When the input is opcode-shuffled — a host has permuted the instruction numbering — an inference stage runs first: detectors pin the opcodes they can prove from structure, bijection completes the rest, and the permutation is applied before lifting. That path is inherently less certain than a canonical or database-backed decode, so every remapped decode is stamped with an evidence header recording what was pinned versus inferred (see [Correctness](#correctness)). The same engine is exposed three ways — the CLI, the `luau-core` crate, and the `luau-ffi` C ABI — so the analysis is identical no matter how you call it.
