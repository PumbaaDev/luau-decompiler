...
#[command(
    ...
    long_about = "A fast, fully offline Luau bytecode decompiler.\n\
                   Supports bytecode v3-9 | Windows, macOS, Linux.\n\n\
                   Quick start:\n  \
...
enum Commands {
    ...
    /// Decompile ONE file and report every semantic finding (Wrong + Suspicious)
    ///
    /// The batch command writes SEMANTIC_REPORT.tsv / SEMANTIC_SUSPECT.tsv over a
    /// whole corpus; this runs the same checks on a single file so a defect can
    /// be located without setting up a batch directory. Wrong findings are
    /// provable defects; Suspicious findings are dropped-value / gaming-risk
    /// candidates that never count toward the corpus defect rate.
    Diagnose {
        /// Bytecode file (- for stdin)
        input: Option<PathBuf>,
        /// Also print the decompiled source above the findings
        #[arg(long)]
        source: bool,
    },
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Diagnose { input, source }) => {
            use luau_core::decompiler::semantic_check::{check, Severity};
            let data = read_input(input.as_deref())?;
            let src = decompile_with_store(&data, store.as_ref(), &db)?;
            if source {
                println!("{src}");
                println!("{}", "-".repeat(72));
            }
            // Proto count from the header comment, same source the batch uses.
            let protos = src.lines().take(20).find_map(|l| {
                l.trim().strip_prefix("-- Protos:")?.trim().split_whitespace().next()?.parse().ok()
            });
            let findings = check(&src, protos);
            let wrong: Vec<_> = findings.iter().filter(|f| f.severity == Severity::Wrong).collect();
            let suspect: Vec<_> =
                findings.iter().filter(|f| f.severity == Severity::Suspicious).collect();
            let name = input
                .as_deref()
                .map(|p| p.display().to_string())
                .unwrap_or_else(|| "<stdin>".to_string());
            println!(
                "{name}: {} provable defect(s), {} suspicious candidate(s)",
                wrong.len(),
                suspect.len()
            );
            let print_group = |label: &str, group: &[&luau_core::decompiler::semantic_check::Finding]| {
                if group.is_empty() {
                    return;
                }
                println!("\n{label}:");
                for f in group {
                    let line = f.line.map(|n| n.to_string()).unwrap_or_else(|| "-".into());
                    let func = enclosing_function(&src, f.line).unwrap_or_else(|| "-".into());
                    println!("  [{}] {}:{}  ({})\n      {}", f.check, name, line, func, f.detail);
                }
            };
            print_group("WRONG (provable defects)", &wrong);
            print_group("SUSPICIOUS (dropped-value / gaming-risk candidates)", &suspect);
            if wrong.is_empty() && suspect.is_empty() {
                println!("no findings — no provable defect and no suspicious candidate.");
            }
        }
        ...
        Some(Commands::Disassemble { input, debug_info, opmap }) => {
            ...
            let text = if opmap {
                ...
                //
                // The DATABASE has to be resolved here too, with the same
                // DB-first precedence as `decompile_with_store`. It was not,
                // so on any chunk a measured entry covers this printed a
                // detector map the decompile never touched — the case where
                // the listing is most likely to be believed.
                if let Some(entry) = resolve_db_entry(&db, &data)? {
                    luau_core::disassemble_with_plan(
                        &data,
...
/// then to a repo-bundled measured database if one is present.
///
/// The bundled fallback is existence-guarded and read-only, and the database
/// itself only ever installs an entry that ANCHOR-MATCHES the chunk (a miss is a
/// no-op — see `opmap_db`), so shipping a verified entry can only make matching
/// files decode from a measurement instead of a guess; it can never corrupt a
/// chunk from a different client build. This is what turns the measured
/// permutation into the DEFAULT decode path for the client build it describes,
/// rather than something every invocation must opt into by flag.
fn db_path(cli: &Cli) -> Option<PathBuf> {
    cli.opmap_db
        ...
        .or_else(bundled_db_path)
}

/// A measured-permutation database bundled with the repository, if present.
///
/// Checked relative to the current directory (how `batch` is normally run) and
/// relative to the executable, so it is found whether the tool runs from the
/// repo root or from an installed location. Returns `None` when no bundled
/// database exists, which restores the exact prior behaviour.
fn bundled_db_path() -> Option<PathBuf> {
    const REL: &str = "opmap_db/roblox_v9_live.json";
    let mut candidates: Vec<PathBuf> = vec![PathBuf::from(REL)];
    if let Ok(exe) = std::env::current_exe() {
        // exe lives at target/<profile>/luau-decompiler.exe; the repo root is
        // two directories up.
...
fn run_batch(
    ...
    for path in files {
        ...
        // A leading 0 byte is not a version. Luau writes it, followed by a
        // compiler diagnostic, when the SOURCE failed to compile, and Roblox
        // ships that blob in place of bytecode. There is no program in it.
        //
        // Counting it as a decompile failure blames us for the game's broken
        // script. One corpus module is 61 bytes and reads
        // ":16: Expected identifier when parsing method name, got 'end'".
        if data.first() == Some(&0) {
            not_a_program += 1;
            let diag = String::from_utf8_lossy(&data[1..])
                .trim_matches(|c: char| c.is_control() || c == '\u{0}')
                .to_string();
            eprintln!("  not a program: {} -- source failed to compile in-game: {}", stem, diag);
            continue;
        }
    ...
    if not_a_program > 0 {
        eprintln!(
            "Done: {} ok, {} failed, {} not a program ({:.1}s)  [{}/{} decodable]",
            ok, fail, not_a_program, elapsed.as_secs_f64(), ok, ok + fail
        );
    } else {
    ...
    let info = run_manifest::RunInfo {
        ...
        total_inputs: (ok + fail + not_a_program) as usize,
        ok,
        failed: fail,
        not_a_program,
...
/// The function a 1-based output line sits inside: the nearest preceding
/// declaration. Emitted Luau uses `function tbl.Name(`, `local function Name(`
/// and `Name = function(`, and nested closures mean NEAREST is the right
/// answer, not outermost.
fn enclosing_function(src: &str, line: Option<usize>) -> Option<String> {
    let line = line?;
    let lines: Vec<&str> = src.lines().collect();
    if line == 0 || line > lines.len() {
        return None;
    }
    for raw in lines[..line - 1].iter().rev() {
        let t = raw.trim_start();
        let rest = t
            .strip_prefix("local function ")
            .or_else(|| t.strip_prefix("function "));
        if let Some(r) = rest {
            let n: String = r.chars().take_while(|c| *c != '(').collect();
            if !n.trim().is_empty() {
                return Some(n.trim().to_string());
            }
        }
        if let Some(eq) = t.find(" = function(") {
            let n = t[..eq].trim().trim_start_matches("local ").trim();
...
///
/// Also writes `SEMANTIC_REPORT.tsv` beside the output: one row per defect,
/// naming the FILE and LINE. The summary alone counts defects without saying
/// where they are, and a count with no file list cannot be acted on or
/// re-checked later — a defect ledger kept that way goes stale silently and
/// then gets quoted as if it were current. The per-file rows are the evidence;
/// the summary in MANIFEST.md is derived from exactly these rows.
fn summarise_semantics(dir: &Path) -> Option<run_manifest::SemanticSummary> {
    ...
    // One row per defect: file / check / line / detail.
    let mut report = String::from("file\tproto\tcheck\tline\tdetail\n");
    // SUSPICIOUS findings are diagnostic candidates, NOT counted as defects and
    // NOT part of the clean %. They go to a separate ledger so a reviewer can
    // find dropped-value / gaming-risk sites (e.g. dropped_arm_value) without the
    // Wrong-based defect count moving. Written to SEMANTIC_SUSPECT.tsv.
    let mut suspect = String::from("file\tproto\tcheck\tline\tdetail\n");
    let mut suspect_files = 0usize;
    let mut suspect_total = 0usize;
    ...
    for p in &entries {
        ...
        let all = check(&src, protos);
        let name = p.file_name().unwrap_or_default().to_string_lossy().into_owned();
        // Suspicious rows: separate ledger, never affects clean/defect counts.
        let suspicious: Vec<_> =
            all.iter().filter(|f| f.severity == Severity::Suspicious).collect();
        if !suspicious.is_empty() {
            suspect_files += 1;
            suspect_total += suspicious.len();
            for f in &suspicious {
                let detail = f.detail.replace(['\t', '\n', '\r'], " ");
                let line = f.line.map(|n| n.to_string()).unwrap_or_else(|| "-".to_string());
                let proto = enclosing_function(&src, f.line).unwrap_or_else(|| "-".to_string());
                suspect.push_str(&format!("{}\t{}\t{}\t{}\t{}\n", name, proto, f.check, line, detail));
            }
        }
        let wrong: Vec<_> =
            all.into_iter().filter(|f| f.severity == Severity::Wrong).collect();
        if wrong.is_empty() {
            clean += 1;
            continue;
        }
        ...
        for f in &wrong {
            *seen.entry(f.check).or_insert(0) += 1;
            // Tabs and newlines inside a detail would forge extra columns and
            // rows, so a malformed message cannot corrupt the table.
            let detail = f.detail.replace(['\t', '\n', '\r'], " ");
            let line = f.line.map(|n| n.to_string()).unwrap_or_else(|| "-".to_string());
            // WHICH FUNCTION. Without it a defect can only be joined to a
            // lifter trace by FILE and REGISTER NUMBER, and register numbers
            // repeat across protos: one case has a reg 3 in `playerTrace` the
            // gate ADMITS and a different reg 3 elsewhere that is the actual
            // defect, which reads as "admitted and still broken" - a
            // contradiction that does not exist. Pairing by an identifier the
            // print does not carry is the most repeated mistake in this
            // project's journal.
            let proto = enclosing_function(&src, f.line).unwrap_or_else(|| "-".to_string());
    ...
    // A failed write must not be silent: without this file the run's defect
    // count cannot be traced back to any file, so say so rather than leaving
    // a caller to assume the report exists.
    if let Err(e) = fs::write(dir.join("SEMANTIC_REPORT.tsv"), &report) {
        eprintln!("warning: could not write SEMANTIC_REPORT.tsv: {e}");
    }
    // Suspicious ledger: diagnostic candidates (dropped-value / gaming-risk
    // sites), separate from the defect count. Always written so a reviewer can
    // find what the Wrong-only report deliberately excludes.
    if let Err(e) = fs::write(dir.join("SEMANTIC_SUSPECT.tsv"), &suspect) {
        eprintln!("warning: could not write SEMANTIC_SUSPECT.tsv: {e}");
    }
    if suspect_total > 0 {
        eprintln!(
            "Suspect: {} finding(s) across {} file(s) -> SEMANTIC_SUSPECT.tsv (diagnostic only, not in the defect count)",
            suspect_total, suspect_files
        );
    }
...
/// Live Roblox clients ship v10-v14 bytecode with a shuffled opcode table and
/// call-feedback (CALLFB) opcodes. The parser gates v10+ behind two env vars so
/// canonical-upstream callers keep the old behaviour; here, on the user-facing
/// decode path, we flip both on automatically for a v10-14 chunk. v3-9 is left
/// untouched (both flags are version-gated to >= 10 inside the parser), so this
/// cannot change any existing decode. RAW mode is correct because the CLI decodes
/// Roblox client bytecode, whose CALLFB is un-shuffled and neutralised by the
/// measured opmap DB entry rather than by the upstream fixup.
fn enable_v10plus_for(data: &[u8]) {
    if matches!(data.first(), Some(10..=14)) {
        if std::env::var_os("LUAU_ALLOW_V10PLUS").is_none() {
            std::env::set_var("LUAU_ALLOW_V10PLUS", "1");
        }
        if std::env::var_os("LUAU_V10PLUS_RAW").is_none() {
            std::env::set_var("LUAU_V10PLUS_RAW", "1");
        }
    }
}
