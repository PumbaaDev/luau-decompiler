...
//! a corpus module under `opmap_db/roblox_v9_live`
...
//! exactly A == B. Measured across the corpus: 69 of the 70
...
/// the sample module still emits the artifact. Asserting against the bare API
...
fn stickerplacer_reads_the_field_it_calls() {
    let Some(src) = decompile_named("sample_module") else {
        eprintln!("SKIP: sample_module.bin not in corpus");
        return;
    };
    ...
    assert!(
        !artifact,
        "The sample module emitted `GetCanvasTouchPoint = \"GetCanvasTouchPoint\"` - the \
    ...
    assert!(
        reads_field,
        "The sample module no longer reads `.GetCanvasTouchPoint` off a table at all - the \
...
fn a_table_may_store_itself_in_its_own_field() {
    ...
    // The sample module itself need not contain one; scan the corpus for the idiom.
    let dir = corpus_dir();
    if !dir.exists() {
