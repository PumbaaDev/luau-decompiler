...
use super::{
    ...
    simplify_expr,
    premateralize_branch_escapes_split,
    store_complex,
    table_expr,
...
/// `pub(super)` since round 4: the branch-seed pass in `mod.rs` must mint
/// names OUTSIDE this family (a `vN` seed is folded chunk-wide by
/// `free_var_decls` and treated as disposable by cleanup passes — the
/// compile failure on a corpus module).
pub(super) fn is_generic_placeholder(name: &str) -> bool {
...
/// Recognise a multi-term short-circuit guard: a run of consecutive
/// `JUMPIF`/`JUMPIFNOT` converging on exactly two pcs, the guard BODY and the
/// construct EXIT.
///
/// Returns `(condition, body_start, exit)` where `condition` is true exactly
/// when the body runs. Deliberately narrow: at least THREE terms (two is the
/// ordinary `a and b` the existing path already handles), every term but the
/// last targeting the body, the last targeting the exit, and both pcs inside
/// the range being lifted. Anything else returns None and the caller behaves
/// exactly as before.
fn compose_guard_chain(
    code: &[u32],
    regs: &[RegVal],
    pc: usize,
    end: usize,
) -> Option<(Expr, usize, usize)> {
    let mut terms: Vec<(bool, usize, usize)> = Vec::new();
    let mut w = pc;
    while w < end && w < code.len() {
        let op = LuauOpcode::from_u8(insn_op(code[w]));
        if !matches!(op, LuauOpcode::JumpIf | LuauOpcode::JumpIfNot) {
            break;
        }
        let t = (w as i64 + insn_d(code[w]) as i64 + 1) as usize;
        terms.push((op == LuauOpcode::JumpIf, insn_a(code[w]) as usize, t));
        w += 1;
    }
    if terms.len() < 3 {
        return None;
    ...
    let body = w;
    let (last_is_if, last_reg, last_target) = *terms.last()?;
    if last_target <= body || last_target > end {
        return None;
    }
    // Every term but the last must jump to the body.
    if terms[..terms.len() - 1].iter().any(|t| t.2 != body) {
        return None;
    }
    // FIRE condition of a term: JUMPIFNOT fires on `not r`, JUMPIF on `r`.
    let fire = |is_if: bool, reg: usize| -> Expr {
        let r = reg_expr_ref(regs, reg);
        if is_if {
            r
        } else {
            Expr::UnOp { op: UnOp::Not, operand: Box::new(r) }
        }
    };
    let mut cond: Option<Expr> = None;
    for &(is_if, reg, _) in &terms[..terms.len() - 1] {
        let t = fire(is_if, reg);
    ...
    // The final jump SKIPS the body, so the body runs when it does NOT fire.
    let last_fire = fire(last_is_if, last_reg);
    let last_term = Expr::UnOp { op: UnOp::Not, operand: Box::new(last_fire) };
    let cond = match cond {
        None => last_term,
        Some(c) => Expr::BinOp {
            left: Box::new(c),
            op: BinOp::Or,
            right: Box::new(last_term),
        },
    };
    Some((cond, body, last_target))
}

/// `reg_expr` without the &mut - the chain only READS its test registers.
fn reg_expr_ref(regs: &[RegVal], idx: usize) -> Expr {
    match regs.get(idx) {
...
/// May `op` appear in a foldable type-guard span?
///
/// A type-guard span is a short straight-line computation whose result feeds a
/// conditional jump — the `x:IsA(T)` / `type(x)` idiom. Only value setup plus
/// the guard's own call is allowed; a `SET*`, table build, closure or capture
/// is a real side effect that must stay a statement in the arm, so it rejects
/// the fold.
fn is_guard_span_op(op: LuauOpcode) -> bool {
    matches!(
        op,
        LuauOpcode::Nop
            | LuauOpcode::LoadNil
...
/// Within a then-arm `[arm_start, then_end)`, is the leading run of value
/// computation a short-circuit `and` term — a call whose result gates the arm
/// by jumping to the arm's own else on failure?
///
/// `if x and x:IsA(T) then keep else make end` compiles to two `JUMPIFNOT`s
/// that BOTH skip to the else; lifted one jump at a time the second — whose
/// target is the else, one past this arm's end — is silently dropped and the
/// `x:IsA(T)` call parks as a dead `local`, losing the type check. This spots
/// that inner guard so the caller can fold it back into the condition.
///
/// Returns `(guard_pc, is_jumpif, tested_register)`. Deliberately narrow: the
/// span before the jump must be pure value setup ending in a call that DEFINES
/// the tested register (`is_guard_span_op`), and the FIRST control-flow op seen
/// must be that guard jump to `else_start` — anything else is real nested
/// control flow and returns None, so an ordinary `a and b` register test (no
/// call, empty span) or a genuine inner branch is left untouched.
fn find_leading_guard_jump(
    code: &[u32],
    arm_start: usize,
    then_end: usize,
    else_start: usize,
    ...
    while w < then_end && w < code.len() {
        let op = LuauOpcode::from_u8(insn_op(code[w]));
        match op {
            LuauOpcode::JumpIf | LuauOpcode::JumpIfNot => {
                let t = (w as i64 + insn_d(code[w]) as i64 + 1) as usize;
                let reg = insn_a(code[w]) as usize;
                // The tested register must be the value this span just computed
                // with a call: the guard idiom, not a bare second register test.
                if t == else_start && saw_call && last_def == Some(reg) && defines_reg {
                    return Some((w, op == LuauOpcode::JumpIf, reg));
                }
                return None;
            }
            _ => {}
        }
        if !is_guard_span_op(op) {
            return None;
        }
        if matches!(op, LuauOpcode::NameCall | LuauOpcode::Call) {
            saw_call = true;
        }
        if insn_defines_reg_a(op) {
            defines_reg = true;
            last_def = Some(insn_a(code[w]) as usize);
        }
        w += if op.has_aux() { 2 } else { 1 };
    }
    None
}

/// Recover the value a folded guard span left in `g_reg` as an expression,
/// so it can move into the `if` condition. The lift of the span either parks
/// the call in the register (no statement) or, because its only reader — the
/// guard jump — is out of range and gets dropped, materialises it as a single
/// dead `local n = <call>`. In the latter case the dead declaration is removed
/// (nothing reads it) and its value returned. Any other shape returns None and
/// the caller abandons the fold.
fn recover_guard_expr(
    stmts: &mut Vec<Stat>,
    regs: &[RegVal],
    g_reg: usize,
    before_len: usize,
) -> Option<Expr> {
    match regs.get(g_reg) {
        Some(RegVal::Expr(Expr::Name(n))) => {
            let n = n.clone();
            if stmts.len() == before_len + 1 {
                if let Some(Stat::Local { names, values }) = stmts.last() {
                    if names.len() == 1
                        && values.len() == 1
                        && names[0] == n
                        && !expr_references_name(&values[0], &n)
                    {
                        let v = values[0].clone();
                        stmts.truncate(before_len);
                        return Some(v);
                    }
                }
            }
        ...
        Some(RegVal::Expr(e)) if stmts.len() == before_len => Some(e.clone()),
        _ => None,
    }
}

/// Does `[start, end)` contain an observable side effect — a call or a store?
///
/// Used to keep the type-guard fold safe. Folding the guard's call into the
/// condition is only sound if the resulting `if` will SURVIVE: a branch whose
/// arms merely select a value into a register can be dropped by a later pass
/// once that register turns out dead, and dropping `if x and x:m() then r=a
/// else r=b end` degrades the short-circuit call `x:m()` into an unconditional
/// one that errors when `x` is nil (`a measured case`). Requiring a real side effect in
/// an arm means the branch keeps its shape and the guard stays protected.
fn range_has_side_effect(code: &[u32], start: usize, end: usize) -> bool {
    let mut pc = start;
...
/// as it did before.
/// Where a constant-test arm really ends.
///
/// `[arm_start, target)` is the fallthrough arm of a `JumpXEqK*`. A conditional
/// branch INSIDE it whose target lies past `target` is the construct's own
/// continuation, and `lift_instruction_range` only lifts a branch when
/// `target <= end` - so cutting the arm at the first target strands it, and in
/// a loop it degrades to a bare `continue`.
///
/// `case A Cleanup` loses nine of ten calls to this:
///
///     19: JUMPXEQKS R6 ==     "string" -> 23      arm = [21, 23)
///     21: JUMPXEQKS R6 not == "number" -> 26      PAST 23 -> stranded
///     23..25: JUMP -> 103                         the "is number" path: continue
///     26..102: the real cleanup work, 9 calls
///
/// Extending to 26 puts the branch back in range and `detect_else_skip` finds
/// the `JUMP -> 103` at pc 25, resolving both arms.
///
/// Gated hard: the swallowed region must end in a `Return`, or in an
/// unconditional JUMP whose target is EXACTLY the enclosing range end. The
/// second condition is what makes it a continue-to-the-back-edge rather than an
/// arbitrary forward jump; without it this same extension demoted a real
/// `:IsA("BasePart")` guard in `case B` to a discarded local.
fn extended_arm_end(code: &[u32], arm_start: usize, target: usize, end: usize) -> usize {
    if target <= arm_start || target > end {
        return target;
    }
    let mut w = arm_start;
    let mut furthest = target;
    while w < target && w < code.len() {
        let wop = LuauOpcode::from_u8(insn_op(code[w]));
        let wsz = if wop.has_aux() { 2 } else { 1 };
        if matches!(
            wop,
            LuauOpcode::JumpIf
    ...
    // A CONDITION CHAIN, NOT A CONSTRUCT BOUNDARY.
    //
    // If another conditional inside the swallowed region converges on the SAME
    // exit as the escaping branch, the two are terms of one compound condition
    // and the "arm" between them is a short-circuit step, not a body.
    // `case B CameraUtils`:
    //
    //     174: JUMPIF    R10 -> 176     outer; arm = [175, 176)
    //     175: JUMPIFNOT R11 -> 186     escaping branch
    //     176..179: (..):IsA("BasePart")
    //     180: JUMPIFNOT R16 -> 186     <- SAME exit
    //     186..188: else
    //
    // that is `(a or b) and x:IsA("BasePart")`. Extending merges the terms, and
    // the `:IsA` result stops being the condition and becomes a discarded
    // local - the call still prints, so no call counter sees it.
    //
    // Only when the extension would leave a TRAILING BLOCK. If `furthest ==
    // end` the escaping branch targets the very end of the range, so extending
    // just makes the arm the remainder and there is nothing after it to
    // mis-assign - `a measured case` needs that and is wrecked without it,
    // turning `if arg3 then if not File2 then return false end end` into
    // `if arg3 then return false`, which aborts whenever arg3 is set.
    if furthest < end {
        let mut c = target;
        while c < furthest && c < code.len() {
            let cop = LuauOpcode::from_u8(insn_op(code[c]));
            if matches!(
                cop,
                LuauOpcode::JumpIf
                    | LuauOpcode::JumpIfNot
                ...
                let ct = (c as i64 + insn_d(code[c]) as i64 + 1) as usize;
                if ct == furthest {
                    if std::env::var("LUAU_XARM_TRACE").is_ok() {
                        eprintln!(
                            "XARM-CHAIN arm_start={} target={} end={} furthest={} conv_pc={} -> blocked",
                            arm_start, target, end, furthest, c
                        );
                    }
                    return target;
                }
            }
            c += if cop.has_aux() { 2 } else { 1 };
        }
    }
    let mut k = target;
    let mut last: Option<(usize, LuauOpcode)> = None;
    while k < furthest && k < code.len() {
        let kop = LuauOpcode::from_u8(insn_op(code[k]));
        last = Some((k, kop));
        k += if kop.has_aux() { 2 } else { 1 };
    }
    let out = match last {
        Some((_, LuauOpcode::Return)) => furthest,
        // The swallowed region must NOT REJOIN inside this range - the same
        // property `Return` has. A jump landing at or past `end` leaves the
        // range (in `case A` it is the loop back-edge at 103, while the
        // enclosing Linear region ends at 61), so the region is a terminal skip
        // path and the arm genuinely continues past it. A jump landing INSIDE
        // the range rejoins it, which is ordinary code - extending through that
        // is what demoted a real `:IsA("BasePart")` guard in `case B`.
        Some((jpc, LuauOpcode::Jump)) => {
            let m = (jpc as i64 + insn_d(code[jpc]) as i64 + 1) as usize;
            if m >= end { furthest } else { target }
        }
        Some((jpc, LuauOpcode::JumpX)) => {
            let m = (jpc as i64 + insn_e(code[jpc]) as i64 + 1) as usize;
            if m >= end { furthest } else { target }
        }
        _ => target,
    };
    if std::env::var("LUAU_XARM_TRACE").is_ok() {
        eprintln!(
            "XARM arm_start={} target={} end={} furthest={} last={:?} -> {}",
            arm_start, target, end, furthest, last.map(|l| l.1), out
        );
    }
    out
}
...
fn detect_else_skip(
    ...
    let ok = jump_target > target && jump_target <= end;
    if std::env::var("LUAU_ELSE_TRACE").is_ok() {
        eprintln!(
            "ELSE_SKIP then_start={} target={} end={} last_pc={} jump_target={} -> {}",
            then_start, target, end, last_pc, jump_target,
            if ok { "ELSE" } else { "reject" }
        );
    }
    if ok {
...
/// Does any branch or jump in this proto target `want`?
///
/// Used to tell a genuinely dead tail from one the walk merely stopped short
/// of: an instruction something jumps TO is reachable regardless of the path
/// that was just abandoned.
fn pc_is_branch_target(code: &[u32], want: usize) -> bool {
    let mut pc = 0usize;
    while pc < code.len() {
        let op = LuauOpcode::from_u8(insn_op(code[pc]));
        if is_control_flow_op(op) {
            let t = match op {
                LuauOpcode::JumpX => pc as i64 + insn_e(code[pc]) as i64 + 1,
                _ => pc as i64 + insn_d(code[pc]) as i64 + 1,
            };
            if t == want as i64 {
                return true;
            }
...
/// Forward-looking twin of [`reg_is_open_ref_capture`]: the pc of the first
/// `CLOSEUPVALS` at or after `from_pc` whose A operand closes `reg`
/// (`A <= reg`), or `code.len()` when the capture stays open to the end of
/// the proto.
///
/// That pc is exactly where Luau is again free to rebind the slot; up to it,
/// every write to `reg` is by construction an assignment to the SAME captured
/// variable, never the declaration of a new one.
fn ref_capture_close_pc(code: &[u32], from_pc: usize, reg: usize) -> usize {
    let mut i = from_pc;
    while i < code.len() {
        let insn = code[i];
        let op = LuauOpcode::from_u8(insn_op(insn));
        if op == LuauOpcode::CloseUpvals && (insn_a(insn) as usize) <= reg {
            return i;
        }
        i += if op.has_aux() { 2 } else { 1 };
    }
...
/// A write to a register the premateralizer PINNED must be EMITTED as an
/// assignment to the pinned binding — the pin's whole contract is that the
/// register's value is observable through another route (a post-join read, a
/// closure's upvalue, a loop back-edge), so a register-file-only park is
/// annihilated before that observer sees it (`merge_regs` collapses
/// disagreeing parks to `Unknown`; the inline paths hard-restore the
/// snapshot).
///
/// `store_complex` and the MOVE handler already honor the pin. This helper
/// gives the DIRECT-PARK handlers (GETUPVAL / LOADNIL / GETGLOBAL — writers
/// that never route through `store_complex`) the same path. Measured shape
/// (a corpus module child "OnUpdate"): six nil-seeded locals, both diamond arms
/// filling them purely via GETUPVAL — every arm write was parked, annihilated
/// at the join, and the post-join reads minted six permanently-nil names.
///
/// Returns `true` when it consumed the write. Gates:
///  * the register is pinned AT THIS PC — outside a pin span, parking is the
///    correct, deliberate inlining behavior;
///  * the register currently holds its own bound local (`regs` name ==
///    tracker name) — a pin whose seed never materialized has no binding to
///    assign to, and a stale alias must not become an assignment target;
///  * a self-assign (`n = n`) emits nothing but still counts as consumed, so
///    the caller does not re-park an alias over the binding.
fn pinned_write_consumed(
    ...
    let bound = match regs.get(a) {
        Some(RegVal::Expr(Expr::Name(n))) if locals.current_name(a) == Some(n.as_str()) => {
            n.clone()
        }
        _ => return false,
    };
    // A generic `vN` binding is NOT a safe assignment target. Those names are
    // minted per-register (`reg_expr`'s v{idx} family) with no chunk-wide
    // uniqueness, `free_var_decls` folds every same-named orphan into ONE
    // chunk-top `local vN`, and the artifact-cleanup passes deliberately
    // treat `v\d+` locals as disposable (C10j drops, literal propagation).
    // Routing real dataflow through such a name conflates unrelated
    // variables: on one corpus module the seed "v16" collided with an upvalue and two
    // other protos' mints, a cleanup pass const-propagated a stale `0` into
    // the new reads, and the output gained `(0):TweenPosition(...)` — a
    // compile failure. Falling through keeps the old alias-park behavior for
    // exactly the generic-named minority.
    if is_generic_placeholder(&bound) {
        return false;
    }
    if matches!(&value, Expr::Name(s) if *s == bound) {
        return true;
    }
...
/// Restore the pre-branch register view after an arm, optionally KEEPING values
/// the arm defined into slots that were dead before it.
///
/// Rotation 120 printed the loss: `MERGE renderPoint@49 reg=7 before=Unknown
/// self=Unknown other=Unknown` for a register whose arm did `LOADNIL R7`. The
/// blanket `*regs = regs_snapshot` exists so arm writes cannot corrupt the
/// fall-through view, which is right for a register that path still relies on
/// and wrong for a slot that held NOTHING before the branch.
///
/// MEASURED (rotations 121-122), ADOPTED:
///     clean 1,125 -> 1,128   defects 17 -> 14
///     FIXES corpus file A, corpus file B, corpus file C - exactly the three
///           `UNK_CALL_CLEAR` orphans diagnosed in r117/r118. NO new findings.
///     42 files changed; ifs lost 0, loops lost 0, no code removed anywhere.
///
/// CONSTANTS ARE EXCLUDED, and that exclusion is load-bearing. Keeping them let
/// `collapse_constant_ifs` fold a guard away: with constants in,
/// `a corpus module` lost its entire auto-jump guard, `doAutoJump()` included,
/// twice (534 -> 528 calls, 348 -> 344 ifs). Keeping nils ALONE was separately
/// measured byte-neutral, so constants contribute nothing to the fix and all of
/// the damage.
///
/// `LUAU_ARM_KEEP_TRACE` prints every value kept.
fn restore_regs_keeping_dead_defs(regs: &mut Vec<RegVal>, snapshot: &[RegVal], pc: usize) {
    let trace = std::env::var("LUAU_ARM_KEEP_TRACE").is_ok();
    let arm = std::mem::take(regs);
    regs.extend_from_slice(snapshot);
    for (i, slot) in regs.iter_mut().enumerate() {
        if !matches!(slot, RegVal::Unknown) {
            continue;
        }
        // NO CONSTANTS. A kept constant is what lets `collapse_constant_ifs`
        // fold a guard away: rotation 122 traced `a corpus module` keeping
        // `Nil` into `Step@449` reg 2 and `Number(2.0)` into `?@674` reg 8,
        // and that file lost its whole auto-jump guard. Keeping nils ALONE was
        // separately measured byte-neutral (rotation 121), so constants
        // contribute nothing to the fix and everything to the damage.
        if let Some(v @ RegVal::Expr(e)) = arm.get(i) {
            if matches!(
                e,
            ...
            if trace {
                let t = format!("{:?}", e);
                eprintln!(
                    "ARMKEEP	{}	pc={}	reg={}	value={}",
                    crate::decompiler::lifter::current_dce_proto(),
                    pc,
                    i,
                    &t[..t.len().min(70)]
                );
...
/// Bind an arm's final value for a premateralized MERGE-SEED register to the
/// seed name.
///
/// `premateralize_branch_escapes_split` seeds `local Seed` before a diamond and
/// pins the merge register to `Seed` across the arms so BOTH arms write it and
/// the post-join read is defined. When an arm carries an INTERNAL short-circuit
/// on that register (`x = f(a) or f(b)`), the pin is deliberately WITHHELD for
/// that arm (see mod.rs `DO NOT PIN ACROSS A NESTED SHORT-CIRCUIT`): pinning
/// during the operand lifts would collapse the `or` chain and drop operands.
/// But withholding the pin lets the arm's FINAL collapsed write MINT A FRESH
/// local (`local Text6 = .. or ..`) instead of assigning the seed, so the seed
/// is never written and the merge reads it undefined. The sibling arm is lost
/// too: the fresh mint poisons `current_names[reg]` (not snapshotted per-arm),
/// so its `pinned_write_consumed` name-match guard fails and its write is parked
/// and annihilated. Measured on `a measured case setTile` R6:
///
///     local Text4                       -- seed, never assigned
///     if arg2 then
///         local Text6 = RARITY_COLOR[..] or Color3.new(1,1,1)   -- dropped
///     end                               -- else `Text4 = color` GONE
///     TextColor34.TextColor3 = Text4    -- undefined
///
/// This runs AFTER an arm is fully lifted — the `or` chain has already collapsed
/// cleanly (recognize_or_and_chain ran with the pin withheld, so its operand
/// folds were not sabotaged), so the arm's final value is known and correct. It
/// routes that value through the seed binding:
///  * Case A — the arm's LAST statement is `local Fresh = <value>` and the
///    register now holds `Fresh`: rewrite it to `Seed = <value>`.
///  * Case B — the arm parked its final value as a bare bound name with no
///    statement (a MOVE / GETUPVAL inlined into the register file): append
///    `Seed = <name>` — exactly what the withheld pin would have emitted.
///
/// Also repairs `current_names[reg]` to the seed via `record_name`, which un-
/// poisons the sibling arm's pin guard so it binds the seed on its own.
///
/// Gated to premat merge seeds ONLY: the register must have entered the branch
/// bound to a non-generic named local that is PINNED somewhere in the construct
/// span. Only `premateralize_*` pins registers, and inside a branch diamond a
/// pinned register is a merge seed — an ordinary carried local is never pinned,
/// so never matches. An arm that already bound the seed leaves it in the
/// register and is skipped, so a correctly-lifted diamond is byte-identical.
fn bind_arm_merge_seeds(
    ctx: &DecompileContext,
    regs: &mut [RegVal],
    locals: &mut LocalTracker,
    arm_stmts: &mut Vec<Stat>,
    regs_snapshot: &[RegVal],
    construct_start: usize,
    construct_end: usize,
) {
    let nregs = regs.len().min(regs_snapshot.len());
    for reg in 0..nregs {
        // The seed name the register carried INTO the branch.
        let seed = match &regs_snapshot[reg] {
            RegVal::Expr(Expr::Name(n)) => n.clone(),
            _ => continue,
        };
        if is_generic_placeholder(&seed) {
            continue;
        }
        // Only a premateralized merge seed is pinned inside the construct span.
        let is_merge_seed =
            (construct_start..construct_end).any(|pc| ctx.is_pinned_reg(reg as u8, pc));
        if !is_merge_seed {
            continue;
        }
        // What the arm actually left in the register.
        let arm_final = match &regs[reg] {
            RegVal::Expr(Expr::Name(n)) => n.clone(),
            _ => continue,
        };
        if arm_final == seed {
            // Arm already bound the seed — nothing to do.
            continue;
        }
        // Case A: the arm's last statement declares `arm_final` as a fresh local
        // holding its final value. Rebind that value onto the seed. The value
        // must NOT read `arm_final` (a self-referential `local x = x + 1` would
        // become an undefined read once the declaration is gone).
        let case_a = matches!(
            arm_stmts.last(),
            Some(Stat::Local { names, values })
                if names.len() == 1
                    && values.len() == 1
                    && names[0] == arm_final
                    && !expr_references_name(&values[0], &arm_final)
        );
        if case_a {
            if let Some(Stat::Local { values, .. }) = arm_stmts.last() {
                let value = values[0].clone();
                *arm_stmts.last_mut().unwrap() = Stat::Assign {
                    targets: vec![Expr::Name(seed.clone())],
                    values: vec![value],
                };
                regs[reg] = RegVal::Expr(Expr::Name(seed.clone()));
                locals.record_name(reg, &seed);
            }
            continue;
        }
        // Case B: the arm parked its final value as a bare bound name with no
        // emitted statement. Emit the assignment the withheld pin would have.
        // `is_bound_name` confirms the name is a live local/alias — a pure,
        // side-effect-free reference safe to assign to the seed here (still
        // inside the arm's own scope).
        if locals.is_bound_name(&arm_final) {
            arm_stmts.push(Stat::Assign {
                targets: vec![Expr::Name(seed.clone())],
                values: vec![Expr::Name(arm_final.clone())],
            });
            regs[reg] = RegVal::Expr(Expr::Name(seed.clone()));
            locals.record_name(reg, &seed);
        }
...
pub(super) fn lift_instruction_range(
    ...
    let pass_id = crate::decompiler::lifter::next_pass_id();
    if std::env::var("LUAU_LIFTPATH").is_ok() {
        eprintln!(
            "LIFTPATH	pass={pass_id}	{}	flat	{}..{}",
    ...
    let mut watch_prev: Option<String> = None;
    while pc < end && pc < code.len() {
        ...
        // Register watch: LUAU_REG_WATCH=<n> reports every change to regs[n]
        // with the pc that produced it. Placed at the loop boundary so it sees
        // writes from EVERY handler rather than one site.
        if let Ok(w) = std::env::var("LUAU_REG_WATCH") {
            if let Ok(w) = w.parse::<usize>() {
                if w < regs.len() {
                    let now = format!("{:?}", regs[w]);
                    if Some(&now) != watch_prev.as_ref() {
                        // Proto identity is REQUIRED here: pc numbering restarts
                        // per proto, and correlating a WATCH line with a PC dump
                        // from a different proto is exactly how rotation 47
                        // concluded a table died where it had not.
                        eprintln!(
                            "WATCH pass={} proto={}@{} reg={} before_pc={} value={}",
                            pass_id,
                            proto.debug_name.as_deref().unwrap_or("?"),
                            proto.line_defined,
                            w,
        ...
        let d = insn_d(insn);
        crate::decompiler::walk_cover::note(
            proto.debug_name.as_deref().unwrap_or("?"),
            proto.line_defined,
            pc,
        );
        // LUAU_PC_WINDOW=lo-hi dumps the opcode stream for one pc range, so a
        // value loss localised by LUAU_REG_WATCH can be attributed to the
        // instruction (and therefore the region) that caused it.
        if let Ok(win) = std::env::var("LUAU_PC_WINDOW") {
            if let Some((lo, hi)) = win.split_once('-') {
                if let (Ok(lo), Ok(hi)) = (lo.parse::<usize>(), hi.parse::<usize>()) {
                    if pc >= lo && pc <= hi {
                        eprintln!(
                            "PC proto={}@{} {:>4} {:?} a={} b={} c={}",
                            proto.debug_name.as_deref().unwrap_or("?"),
                            proto.line_defined, pc, op, a, b, c
                        );
                    }
                }
            }
        ...
        match op {
            ...
            LuauOpcode::LoadNil => {
                // A pinned register's nil write is as much a write as any
                // other: emit `<bound> = nil` (and do NOT mark the register
                // as an explicit-nil park — it still holds its binding).
                if pinned_write_consumed(ctx, regs, locals, stmts, a, pc, Expr::Nil) {
                    // consumed
                } else {
            ...
            LuauOpcode::GetGlobal => {
                ...
                if let Some(name) = resolve_global_name(proto, &ctx.chunk.strings, d, aux) {
                    // A global read into a pinned register is a write like any
                    // other — emit it, or the join annihilates it.
                    if !pinned_write_consumed(ctx, regs, locals, stmts, a, pc, Expr::Name(name.clone())) {
                        regs[a] = RegVal::Expr(Expr::Name(name));
                    }
                } else {
                    regs[a] = RegVal::Unknown;
                    crate::decompiler::mint_trace::note_unknown("UNK_GETGLOBAL_UNRESOLVED", a);
                }
            }
            LuauOpcode::SetGlobal => {
            ...
            LuauOpcode::GetUpval => {
                ...
                // A pinned destination register (premateralized branch seed,
                // loop-carried accumulator, or open ref-capture span) must
                // have this write EMITTED: the alias park below never reaches
                // the AST, and inside an arm span the join annihilates it —
                // the RingPulse shape (six GETUPVAL-only arm writes, all
                // lost, six permanently-nil post-join reads).
                if pinned_write_consumed(ctx, regs, locals, stmts, a, pc, Expr::Name(name.clone())) {
                    pc += 1;
                    continue;
                }
                // The store is UNCONDITIONAL. It was guarded from Phase B0.52
                // until the compile gate proved the guard wrong (7/628
                // failures, 6 sharing one signature).
                //
                // The guard skipped the store when R[a] was a declared,
                // non-empty local that was not itself an upvalue alias,
                // reasoning that a misdecoded GETUPVAL must not clobber a
                // B0.51-seeded module-table binding. But registers are
                // function-scoped and reuse is routine: a REAL GetUpval into a
                // previously-declared scratch register is everyday bytecode,
                // and skipping the store leaves the register's STALE expression
                // to be read by whatever consumes it next. RootCamera proto 16:
                //
                ...
                //
                // lifted as `(X < Y)(X)` — a statement beginning with `(`,
                // which the Luau compiler rejects outright ("ambiguous
                // syntax"). Same signature in five more corpus files, with
                // bases like `(new74 + new74)`, `("someString")` and `(0)` on
                // calls and namecalls. When the stale value happened to be a
                // plain name the result still compiled and was silently wrong —
                // the worse outcome.
                //
                // The guard's own history already pointed here: the
                // `holds_upval_alias` disjunct (scratch registers reading two
                // upvalues in a row) and the `reg_is_empty` disjunct (measured:
                // 606 of 1807 hoisted-never-assigned names) each widened the
                // store because the drop was found doing damage. Storing
                // unconditionally completes that arc. If a misdecoded GetUpval
                // ever does clobber a live module-table binding, the damage is
                // a VISIBLE wrong name at the table's use sites — strictly
                // better than an invisible stale expression at the call site.
                ...
                locals.record_alias_name(a, &name);
            }
            LuauOpcode::SetUpval => {
            ...
            LuauOpcode::Mul => {
                if a == b && b == c {
                    ...
                    //  a corpus module (12×): callback closures with `v0 * v0` body, no return
                    //  a corpus module (6×): `math.clamp(table * table, ...)` — nonsensical
                    //  a corpus module: `v10 * v10 * (v10 * v10)` in a field context
                    //  a corpus module: `math.rad(CFrame * CFrame)` — nonsensical
            ...
            LuauOpcode::Pow => {
                if a == b && b == c {
                    ...
                    //  a corpus module: v18 = v18 ^ v18 — self-assignment passthrough
                    //  a corpus module: for i = v6, v3^v3, v5 — nonsensical loop bound
                    //  a corpus module: v0^v0 & "table" — pow AND string
            ...
            // 27 corpus B==C patterns: a corpus module's ReplicatedStorage chains, etc.
            LuauOpcode::And => {
                if b == c {
            ...
            LuauOpcode::Length => {
                ...
                //   `SomeUtil = #v44` — assigning length to module name
            ...
            LuauOpcode::NewTable => {
                ...
                if is_proto_start {
                    ...
                    // Honour a pinned destination, exactly as DupTable now does
                    // (df419e1). NewTable was the last direct-park writer that
                    // ignored the pin -- LoadNil, GetGlobal, GetUpval, DupTable
                    // and Call all honour it -- so a table built in a branch
                    // reached the merge as a pending expression, matched
                    // nothing, and went out with merge_regs' terminal reset
                    // along with every SETTABLEKS that had mutated it in place.
                    let empty = Expr::Table { fields: vec![] };
                    if !pinned_write_consumed(ctx, regs, locals, stmts, a, pc, empty.clone()) {
                        regs[a] = RegVal::Expr(empty);
                    }
                }
            }
            LuauOpcode::DupTable => {
                ...
                // Mirror NewTable: when the cloned template is filled or read on
                // the far side of a branch, a parked pending table is cloned at
                // every read, so a cross-branch SETTABLEKS mutates a throwaway
                // copy and is discarded at the merge (`floor25 extends Amount6`
                // in a measured case). Materialise eagerly in that case so the field
                // write becomes a real `t.Amount = ...` Stat that survives; keep
                // the pending-literal inline path otherwise so short
                // `return {a=1, b=2}` tables still inline. See table_needs_binding.
                if pc <= 1 || table_needs_binding(code, pc, a) {
                    let new_name = ctx.reg_name(proto, a as u8, pc);
                    let (kind, name) = locals.classify_write(a, &new_name);
                    match kind {
                        WriteKind::FirstDecl | WriteKind::Shadow => {
                    ...
                    regs[a] = RegVal::Expr(Expr::Name(name));
                } else if !pinned_write_consumed(ctx, regs, locals, stmts, a, pc, expr.clone()) {
                    // Honour a pinned destination, as LoadNil / GetUpval / GetGlobal
                    // all do. Parking unconditionally ignores a pin that was set
                    // before the branch, so both arms reach the merge holding
                    // structurally different pending tables, neither is a Name, and
                    // `merge_regs` takes its terminal reset -- discarding the table
                    // and every SETTABLEKS that mutated it in place.
                ...
                if is_recursive {
                    ...
                    while cap_pc < code.len() && inferred_upval_names.len() < expected_upvals {
                        ...
                        if is_capture {
                            ...
                            // A REF capture (type 1) whose source register is a
                            // PARAMETER shares that parameter's CELL for the life
                            // of the closure. A parameter register never
                            // re-declares (`classify_write` returns Reassign for
                            // reg < num_params), so its cell identity is the
                            // parameter — but the value tracker can move ahead of
                            // it: `arg = arg or game` overwrites `regs[arg]` with
                            // the global `game`, and the `Some(Expr::Name(n))` arm
                            // above then names the upvalue after that global. A
                            // global/stdlib name can NEVER be the correct name for
                            // a ref-captured local cell, so in that case — and only
                            // that case, to leave every already-correct heuristic
                            // name untouched — bind to the parameter name instead.
                            let name = if cap_type == 1
                                && cap_op == LuauOpcode::Capture
                                && (cap_idx as usize) < proto.num_params as usize
                                && is_stdlib_shadow_name(&name)
                            {
                                ctx.reg_name(proto, cap_idx, 0)
                            } else {
                                name
                            };
                            ...
                            // A register captured by REFERENCE stays the SAME
                            // variable for every write between this CAPTURE and
                            // the CLOSEUPVALS that releases it — the allocator
                            // cannot rebind the slot while the upvalue is open.
                            // The one site that knew this
                            // (`reg_is_open_ref_capture`, guarding closure
                            // stores) left every OTHER write path free to split
                            // the binding: JellyBeanToss's `MOVE R12 R16`
                            // copy-propagated the `RenderStepped:Connect(...)`
                            // result straight past the declared `local result3`
                            // that the child closure Disconnects, leaving the
                            // child reading a never-assigned name (census class
                            // declared_never_assigned, mechanism 1b).
                            //
                            // Pin the register to its bound name for exactly
                            // that span, riding the same machinery
                            // `premateralize_loop_carried` uses for loop-carried
                            // accumulators. The invariant is identical: the
                            // register's value is observable through another
                            // route (here the closure's upvalue), so every
                            // write must be EMITTED as an assignment to one
                            // stable name — never parked in the register file,
                            // never shadow-declared under a fresh name. The pin
                            // pre-seeds `reg_name` across the span, so the CALL
                            // / store_complex / classify_write paths all
                            // resolve to the pinned name and classify their
                            // writes as Reassign; the MOVE handler's
                            // `pinned_dest` arm materializes copy-propagated
                            // stores outright.
                            //
                            // Gates: a REAL remapped CAPTURE only (a structural
                            // guess must not pin); REF kind only (a VAL capture
                            // copies, so a later write to the register is the
                            // allocator reusing a dead slot for a NEW variable,
                            // which a pin would wrongly redirect into the old
                            // one); and the register must currently hold its
                            // bound local (`regs` name == tracker name) — a
                            // register that never got a binding here is the
                            // census's 1a family, not this one.
                            if cap_op == LuauOpcode::Capture && cap_type == 1 {
                                if let Some(RegVal::Expr(Expr::Name(n))) =
                                    regs.get(cap_idx as usize)
                                {
                                    if locals.current_name(cap_idx as usize)
                                        == Some(n.as_str())
                                    {
                                        let close =
                                            ref_capture_close_pc(code, cap_pc, cap_idx as usize);
                                        ctx.pin_reg_name(cap_idx, n, cap_pc + 1, close);
                                    }
                                }
                            }

                            // Record a trusted global-ref capture: the name is one
                            // of the three Roblox instance globals AND the source
                            // register genuinely held that exact global expression
                            // at capture time. `upval_name` uses this to let the
                            // name through the stdlib-shadow guard — safe because
                            // the captured value IS that global.
                            if cap_op == LuauOpcode::Capture
                                && matches!(name.as_str(), "game" | "workspace" | "script")
                            {
                                if let Some(RegVal::Expr(Expr::Name(n))) = regs.get(cap_idx as usize)
                                {
                                    if n == &name {
                                        ctx.capture_global_upvals
                                            .entry(child_global_idx)
                                            .or_default()
                                            .insert(inferred_upval_names.len() as u8);
                                    }
                                }
                            }
                    ...
                    // `a corpus module` where two NEWCLOSUREs to R1 with
            ...
            LuauOpcode::Call | LuauOpcode::CallFb => {
                // CALLFB is normalised to CALL in remap_chunk, but handle it
                // here too so the match stays exhaustive and correct either way.
                ...
                //
                // A corrupted or misdetected opcode yields a garbage C, and
                // believing it makes the lifter capture dozens of registers
                // that were never results. This used to be rejected with a flat
                // `> 10`, on the reasoning that "Luau functions almost never
                // return >10 values" - but `CFrame:GetComponents()` returns
                // exactly 12, and it is everywhere in Roblox code. The clamp
                // could not tell that from garbage, so it called the whole call
                // a statement, and the register clear below then wiped all 12
                // results to Unknown. `a corpus module` returned
                // `{v4, v5, v6, 1, ...}` reading twelve names nothing assigns.
                //
                // The bytecode bounds this exactly, so guessing is unnecessary:
                // the results occupy R(A)..R(A+n-1) and the proto declares how
                // many registers it has. A count that does not fit the frame is
                // genuinely impossible; one that fits is a real call, whatever
                // its size. Rejection still clamps to 1 (statement, 0 captures)
                // rather than 0 (vararg), because vararg mode triggers
                // aggressive register scanning that over-collects stale
                // arguments from previous calls.
                let raw_nresults = c;
                let captured = raw_nresults.saturating_sub(1) as usize;
                let fits_frame = a + captured <= proto.max_stack_size as usize;
                let nresults = if raw_nresults != 0 && !fits_frame { 1 } else { raw_nresults };
                ...
                if is_vararg_call {
                    ...
                    // `max_args` is already the frame bound, so the extra
                    // `.min(10)` was a second guess on top of an exact limit.
                    for i in 0..max_args {
                        let r = start + i;
                        match regs.get(r) {
                            Some(RegVal::Expr(e)) if !matches!(e, Expr::Nil) => {
                    ...
                    // Reject an IMPOSSIBLE argument count, not a large one.
                    //
                    // A corrupted or misdetected opcode yields a garbage B, and
                    // believing it collects dozens of registers that were never
                    // arguments. This used to be a flat `nargs.min(10)` on the
                    // reasoning that "Luau functions rarely have >10 args" - the
                    // same guess that, on the RESULT side, decompiled
                    // `CFrame:GetComponents()` (12 returns) as a bare statement
                    // and wiped every result register.
                    //
                    // The bytecode bounds this exactly: arguments occupy
                    // R(A+1)..R(A+nargs) and the proto declares its frame. A
                    // count that does not fit is genuinely impossible; one that
                    // fits is a real call whatever its size.
                    let fits_frame = a + nargs <= proto.max_stack_size as usize;
                    let capped_nargs = if fits_frame { nargs } else { nargs.min(10) };
                ...
                if nresults == 1 {
                    ...
                    if nresults == 2 {
                        ...
                        } else if pinned_write_consumed(
                            ctx, regs, locals, stmts, a, pc, call_expr.clone(),
                        ) {
                            // Round 4: a call result landing in a PINNED
                            // register is an arm/loop write like any other —
                            // it must be emitted as an assignment to the
                            // pinned identity, not classified into a fresh
                            // arm-local that the join annihilates
                            // (a corpus module: `R2 = tbl.GetLocalPlayer
                            // CanvasSet()` inside the else arm became `local
                            // GetLocalPlayerCanvasSet3 = ...` and the merge
                            // read minted `v2`).
                        } else {
                            // Phase B0.49: classify_write for shadow-on-rename.
                            let (kind, name) = locals.classify_write(a, &new_name);
                ...
                // `nresults == 0` is MULTRET: the results run from `a` to the
                // top of stack and how many there are is decided by whatever
                // consumes them (SETLIST, a following CALL, RETURN). The arm
                // above assumes exactly one "by convention", so everything from
                // a+1 upward is wiped -- including real result registers when
                // the call returned more than one. A later read of one of those
                // then finds Unknown and mints a name nothing assigns.
                //
                // Clearing on an unknown count is a guess in the destructive
                // direction; leaving them alone is a guess in the recoverable
                // one. MEASURED, and it does not pay: skipping the clear for
                // MULTRET moved CoreGui from 1699/303 to 1700/305 -- one more
                // file clean, two more defects, i.e. noise -- while leaving
                // stale values in registers the VM considers dead. Kept as the
                // clear it was; the note stands so the next reader does not
                // re-run the experiment.
                // NOTE: preserving registers that hold a declared local was
                // measured (rotation 42) and REVERTED. It raised the score to
                // 1,125/22 and corrupted loop variables: keeping the binding
                // alive made later temporaries write THROUGH that local's name,
                // so a corpus module emitted `i = someExpr...`,
                // `i = i.CFrame` over its own `for` variable.
                if std::env::var("LUAU_FRAME_TRACE").is_ok() {
                    let hi = locals.declared.iter().copied().max().unwrap_or(0);
                    let live_above: Vec<usize> = locals
                        .declared
                        .iter()
                        .copied()
                        .filter(|&r| r >= clear_start)
                        .collect();
                    if !live_above.is_empty() {
                        eprintln!(
                            "FRAME pc={} A={} clear_start={} maxstack={} highest_declared={} DECLARED_ABOVE={:?}",
                            pc, a, clear_start, proto.max_stack_size, hi, live_above
                        );
                    }
                }
                for r in clear_start..regs.len() {
                    regs[r] = RegVal::Unknown;
                    // CORRECT by construction: a call leaves everything above
                    // its results genuinely holding nothing. Tagged so this can
                    // be counted APART from the give-up paths -- suppressing a
                    // declaration whose register was cleared here would replace
                    // a visible defect with a silent wrong value.
                    crate::decompiler::mint_trace::note_unknown("UNK_CALL_CLEAR", r);
            ...
            LuauOpcode::Return => {
                ...
                //
                // Guard on the PROTO tail, not merely the lifted range end: a
                // bare early `return` inside a guard (`if x then return end`)
                // is lifted as its own sub-range and would otherwise satisfy
                // `pc + 1 >= end` — the range end cannot tell the function's
                // implicit final RETURN from a real early return that merely
                // sits at a sub-range boundary. `a measured case` drops
                // two in-loop `return` guards this way once the generic-for is
                // recovered and the guards become their own InlineIfThenInLoop
                // sub-ranges.
                let is_trailing_empty =
                    values.is_empty() && pc + 1 >= end && end >= proto.code.len();
                if !is_trailing_empty {
                    stmts.push(Stat::Return { values });
                }
                // A Return terminates its BLOCK, not necessarily the span.
                // Same fault as the unconditional-jump arm (rotation 88), third
                // variant. `a corpus module::OnInputBegin`:
                //
                //     15: JUMPIFNOT R2 -> 17    `if pulling or dragging then`
                //     16: RETURN                    return end
                //     17: GETTABLEKS R2 R1."UserInputType"
                //
                // pc 17 is the JUMPIFNOT's own target, so it is reachable; but
                // breaking at 16 abandoned 17..48 and cost the function 30% of
                // its instructions (`LUAU_WALK_COVER`: walked 106 of 150).
                //
                // Resume only when something else in the proto jumps to the
                // next instruction, which is precisely when the tail is live.
                //
                // MEASURED (rotation 90) and NOT adopted - it is the right
                // mechanism with no demonstrable payoff:
                //
                //     clean 1,106 -> 1,106   defects 37 -> 37
                //     defect set byte-identical: nothing fixed, nothing broken
                //     OnInputBegin coverage 70% -> 82% (walked 106 -> 123)
                //     30 files changed; corpus total 284,717 -> 284,710 lines,
                //     so the extra instructions produced no statements and no
                //     duplication either
                //
                // r87 and r88 each closed a defect with clear recoveries. This
                // changes 30 files to move nothing the checker can see, so it
                // stays gated until there is a finding it actually fixes. The
                // coverage number is kept because the mechanism is right and
                // the next attempt on `calls_not_emitted` starts here.
                let next_pc = pc + if op.has_aux() { 2 } else { 1 };
                if std::env::var("LUAU_RET_RESUME").is_ok()
                    && next_pc < end
                    && pc_is_branch_target(code, next_pc)
                {
                    pc = next_pc;
                    continue;
                }
            ...
            LuauOpcode::Jump | LuauOpcode::JumpBack | LuauOpcode::JumpX => {
                ...
                if target > pc && target <= end {
                    ...
                    if std::env::var("LUAU_SPANBREAK").is_ok() {
                        eprintln!(
                            "SPANJUMP	{}@{}	pc={}	target={}	span={}..{}	exits={}",
                            proto.debug_name.as_deref().unwrap_or("?"),
                            proto.line_defined,
                            pc,
                            target,
                    ...
                    // "Remaining instructions are dead code" is only true when
                    // the span was carved correctly. Measured false on
                    // `a corpus module::ConstructCell`: the span 22..99
                    // holds `57: JUMP -> 216`, and pc 58 is the OTHER side of a
                    // conditional above it - reachable, not dead. Breaking
                    // there abandoned 58..98, so `98: CALL R19` never ran and
                    // the `JUMPIFNOT R19` at 99 emitted a bare `v19`.
                    //
                    // Resume instead when something else in the proto jumps to
                    // the next instruction, which makes it reachable
                    // independently of the path just abandoned.
                    //
                    // MEASURED (rotation 88), ADOPTED:
                    //     clean 1,105 -> 1,106   defects 38 -> 37
                    //     fixes a corpus module; NO new findings
                    //     7 files changed; EVERY delta non-negative -
                    //     calls gained in six, nothing lost anywhere
                    let next_pc = pc + if op.has_aux() { 2 } else { 1 };
                    if next_pc < end && pc_is_branch_target(code, next_pc) {
                        pc = next_pc;
                        continue;
                    }
                    // Rotation 110: the span bounds at the moment the walk
                    // gives up on an out-of-range jump. This family has been
                    // diagnosed four times without ever printing the ONE number
                    // that says whose span is too short.
                    if std::env::var("LUAU_SPANBREAK").is_ok() {
                        eprintln!(
                            "SPANBREAK	{}@{}	pc={}	target={}	span={}..{}	next={}	is_target={}",
                            proto.debug_name.as_deref().unwrap_or("?"),
                            proto.line_defined,
                            pc,
                            target,
            ...
            LuauOpcode::JumpIf | LuauOpcode::JumpIfNot => {
                // MULTI-TERM SHORT-CIRCUIT GUARD.
                //
                // `if not (a and b and c and d) then .. end` compiles to a RUN
                // of conditional jumps converging on two pcs - the guard body
                // and the construct's exit. Lifted one jump at a time, each
                // term becomes its own tiny `if` and NO SINGLE `if` OWNS THE
                // BODY, so the body lands at the outer level. When that body
                // ends in `return`, `eliminate_dead_code` then correctly cuts
                // everything after it. `a measured case IndexController` proto #9 loses
                // 30 of its 52 calls exactly this way:
                //
                //     136: JUMPIFNOT R13 -> 140   fires when !R13, jumps to BODY
                //     137: JUMPIFNOT R14 -> 140
                //     138: JUMPIFNOT R15 -> 140
                //     139: JUMPIF    R16 -> 145   fires when R16, jumps to EXIT
                //     140..144: warn(..) ; RETURN
                //
                // The body runs on `!R13 or !R14 or !R15 or !R16`. The rule:
                // OR the FIRE conditions of every jump targeting the body, then
                // OR the NEGATED fire condition of the final jump, which is the
                // one that skips the body.
                if let Some((chain_cond, body, exit_pc)) =
                    compose_guard_chain(code, regs, pc, end)
                {
                    let snap = regs.clone();
                    let mut then_stmts = Vec::new();
                    lift_instruction_range(
                        ctx, proto, proto_index, depth + 1, body, exit_pc,
                        regs, locals, &mut then_stmts, in_loop,
                    );
                    regs.clear();
                    regs.extend_from_slice(&snap);
                    if !then_stmts.is_empty() {
                        stmts.push(Stat::If {
                            condition: chain_cond,
                ...
                let mut target = (pc as i32 + d as i32 + 1) as usize;
                // The RAW jump target, before `extended_arm_end` may push it out
                // to a later construct end. The type-guard fold only applies when
                // this jump directly skips to the else; a jump that lands INSIDE
                // the then-arm (its target was extended) is an `a or b` short-
                // circuit whose first operand jumps to the body, not an `and`.
                let raw_target = target;

                // THE ARM ENDS WHERE THE CONSTRUCT ENDS, NOT AT THE FIRST TARGET
                // SEEN. A branch inside the arm targeting past it is the
                // construct's own continuation, and stranding it degrades to a
                // bare `break`/`continue` - or, outside a loop, to nothing at
                // all. See `extended_arm_end` for the gate and the two files
                // that forced its exact shape.
                {
                    let arm_start = pc + 1 + if op.has_aux() { 1 } else { 0 };
                    target = extended_arm_end(code, arm_start, target, end);
                }
                ...
                // Rotation 127: the last suspect. r126 cleared premateralization,
                // leaving this handler as the only thing between a correct R3 at
                // the top of pc 17 and a Nil one at the arm lift.
                if std::env::var("LUAU_IDIOM_TRACE").is_ok() {
                    if let Some(id) = recognize_bool_idiom(code, pc) {
                        eprintln!("IDIOM	{}	pc={}	dest={}	taken={}	end_pc={}",
                            crate::decompiler::lifter::current_dce_proto(),
                            pc, id.dest, id.taken_value, id.end_pc);
                    }
                }
                ...
                if let Some(chain) = recognize_or_and_chain(code, pc) {
                    // SPECULATIVE. The segment lifts below write into the SHARED
                    // register file, and when the chain is rejected (`ok = false`)
                    // control falls through to the inline-if path with those
                    // writes still in place.
                    //
                    // Measured on `a corpus module::?@74` (rotations 124-129, all
                    // pass-stamped): R3 holds `Name("SteeringVelocity3")` at the
                    // top of pc 17, a speculative lift of 18..22 runs, and the
                    // enclosing pass then builds its arm with R3 = `Expr(Nil)` -
                    // the value LOADNIL left at pc 10, because the speculative
                    // pass never executed pcs 13..16. `method_receiver_expr`
                    // then correctly refuses a nil receiver and mints `v3`, so
                    // the output calls a method on nothing.
                    //
                    // Snapshot and restore on the reject path only; the accept
                    // path deliberately keeps its writes.
                    let chain_snapshot = regs.clone();
                    ...
                    // Rejected: undo the speculation.
                    //
                    // MEASURED (rotation 130), ADOPTED:
                    //     clean 1,128 -> 1,130   defects 14 -> 12
                    //     FIXES a corpus module (the diagnosed reproducer) and
                    //           a corpus module. NO new findings.
                    //     121 files changed; calls lost 0, loops lost 0;
                    //     total emitted calls 38,459 -> 38,461.
                    //     Two files show one fewer `if` and BOTH are recoveries:
                    //     `a corpus module` collapses a flattened short-circuit into
                    //     `x and (self.Amount or 1) <= #x` (the discarded value
                    //     is now assigned), and `a corpus module` gains a
                    //     17-line table constructor.
                    regs.clear();
                    regs.extend_from_slice(&chain_snapshot);
                }
                ...
                if target > pc && target <= end && !exits_loop {
                    ...
                    let mut condition = if op == LuauOpcode::JumpIfNot {
                    ...
                    // below annihilates the branch's result and the join
                    // silently reads the stale pre-branch literal. Mirrors the
                    // JumpIfEq-family arm. Split arms (round 4) so a register
                    // with NO pre-branch binding written by BOTH arms gets a
                    // seeded identity instead of a minted `vN` at the join —
                    // a corpus module proto 10, whose diamond sits INSIDE the outer
                    // `if arg2.Badges` arm and lifts through this inline path.
                    let construct_end = else_range.map_or(target, |(_, ee)| ee);
                    // Rotation 126: the two-suspect window from r125. Name before and
                    // Nil after implicates premateralization; Nil already before puts
                    // the write in the pc-17 condition handling.
                    if let Ok(w) = std::env::var("LUAU_ARM_ENTRY") {
                        if let Ok(w) = w.parse::<usize>() {
                            eprintln!("PREMAT	BEFORE	pass={}	{}	pc={}	reg={}	value={:?}",
                                pass_id,
                            crate::decompiler::lifter::current_dce_proto(), pc, w, regs.get(w));
                        }
                    }
                    premateralize_branch_escapes_split(
                        ctx, proto, regs, locals, stmts,
                        // AFTER-RANGE IS THE PROTO, NOT THIS LIFT WINDOW.
                        //
                        // `end` is the end of the range currently being lifted -
                        // an arm, a run - so a register read AFTER that window
                        // was invisible here and the seed was rejected as
                        // "not used after merge" even though the read plainly
                        // exists. That rejection is 7 of the 13 `vN`
                        // declared-never-assigned defects on the measured corpus
                        // (case A v5, case B v3, case C v19, case D v11,
                        // case E v3 and v9, case F v15) - the gate declined to
                        // seed a register the semantic checker can see being
                        // read. The usage question is about the whole proto.
                        (jump_next, then_end), else_range, construct_end, proto.code.len(), pc,
                    );

                    if let Ok(w) = std::env::var("LUAU_ARM_ENTRY") {
                        if let Ok(w) = w.parse::<usize>() {
                            eprintln!("PREMAT	AFTER	pass={}	{}	pc={}	reg={}	value={:?}",
                                pass_id,
                            crate::decompiler::lifter::current_dce_proto(), pc, w, regs.get(w));
                        }
                    }
                    ...
                    let mut then_stmts = Vec::new();
                    // Rotation 125: the arm's register state at ENTRY.
                    // r122 instrumented what an arm's writes do on the way OUT;
                    // `a corpus module` shows a receiver arriving as `Nil` on the way
                    // IN, and that has never been printed.
                    if let Ok(w) = std::env::var("LUAU_ARM_ENTRY") {
                        if let Ok(w) = w.parse::<usize>() {
                            eprintln!(
                                "ARMENTRY\tpass={}\t{}\tarm={}..{}\treg={}\tvalue={:?}",
                                pass_id,
                                crate::decompiler::lifter::current_dce_proto(),
                                jump_next,
                                then_end,
                    ...
                    //
                    // `if x and x:IsA(T) then keep else make end` compiles to two
                    // JUMPIFNOTs that BOTH skip to the else: the first tests `x`,
                    // the second tests the `x:IsA(T)` call. This outer jump owns
                    // `if x`; the inner one -- whose target is the else, one past
                    // this arm's end -- is otherwise dropped, parking the call as a
                    // dead `local` and running the arm body unconditionally, so the
                    // type check is lost (`a measured case`, and the corpus-wide
                    // `dropped_arm_value` shape). Fold each such leading guard into
                    // the condition as an `and` term so the check survives and the
                    // arm body starts past it. Gated to the call-producing guard
                    // idiom; kill-switch `LUAU_NO_TG` restores the old behaviour.
                    //
                    // Decide the fold PURELY STRUCTURALLY first, before lifting
                    // anything: the guard-span lift mints names into the shared
                    // context, which cannot be rolled back, so a lift-then-abort
                    // corrupts later naming. Phase 1 walks the guard chain with no
                    // side effects; phase 2 gates it; only then does phase 3 lift.
                    let mut arm_start = jump_next;
                    if std::env::var("LUAU_NO_TG").is_err() {
                        // Only fold when this jump skips DIRECTLY to the else. If
                        // `extended_arm_end` moved the target, the jump lands
                        // inside the then-arm — an `a or b` whose first operand
                        // jumps to the body — and folding it as `and` would drop
                        // that operand's path (`a measured case`).
                        if let Some((else_start, _)) =
                            else_range.filter(|(es, _)| *es == raw_target)
                        {
                            // Phase 1: locate the guard chain (no lifting).
                            let mut probe = jump_next;
                            loop {
                                match find_leading_guard_jump(code, probe, then_end, else_start) {
                                    Some((g, _, _)) if g > probe => probe = g + 1,
                                    _ => break,
                                }
                            }
                            // Phase 2: only fold when an arm carries a real side
                            // effect, so the resulting `if` survives later passes.
                            // A pure value-select branch can be dropped once its
                            // register turns out dead, which would degrade the
                            // short-circuit guard call into an unconditional one
                            // that errors on nil (`a measured case`). See
                            // `range_has_side_effect`.
                            let else_effect = else_range
                                .map_or(false, |(es, ee)| range_has_side_effect(code, es, ee));
                            let do_fold = probe > jump_next
                                && (range_has_side_effect(code, probe, then_end) || else_effect);
                            // Phase 3: lift each guard span and fold its call into
                            // the condition as an `and` term.
                            if do_fold {
                                loop {
                                    let Some((g, g_is_if, g_reg)) =
                                        find_leading_guard_jump(code, arm_start, then_end, else_start)
                                    else {
                                        break;
                                    };
                                    if g <= arm_start {
                                    ...
                                    let before_len = then_stmts.len();
                                    lift_instruction_range(
                                        ctx, proto, proto_index, depth + 1, arm_start, g, regs,
                                        locals, &mut then_stmts, in_loop,
                                    );
                                    match recover_guard_expr(&mut then_stmts, regs, g_reg, before_len)
                                    {
                                        Some(rc) => {
                                            // Body runs when the guard does NOT fire:
                                            // JUMPIFNOT fires on falsy (needs `rc`),
                                            // JUMPIF fires on truthy (needs `not rc`).
                                            let term = if g_is_if {
                                                Expr::UnOp {
                                                    op: UnOp::Not,
                                                    operand: Box::new(rc),
                                            ...
                                            condition = Expr::BinOp {
                                                ...
                                                op: BinOp::And,
                                                right: Box::new(simplify_expr(&term)),
                                            };
                                            arm_start = g + 1;
                                        }
                                        None => {
                                            // Defensive: the structural match said
                                            // this was a clean call guard but the
                                            // lift disagreed. Abandon and lift the
                                            // arm as it was (rare; naming may drift).
                    ...
                    lift_instruction_range(ctx, proto, proto_index, depth + 1, arm_start, then_end, regs, locals, &mut then_stmts, in_loop);
                    bind_arm_merge_seeds(ctx, regs, locals, &mut then_stmts, &regs_snapshot, jump_next, construct_end);
                    ...
                    let else_body = if let Some((es, ee)) = else_range {
                        *regs = regs_snapshot.clone();
                        ...
                        bind_arm_merge_seeds(ctx, regs, locals, &mut else_stmts, &regs_snapshot, jump_next, construct_end);
                        Some(else_stmts)
                    } else {
                        None
                    };

                    restore_regs_keeping_dead_defs(regs, &regs_snapshot, pc);
                    stmts.push(Stat::If {
                        condition,
                        then_body: then_stmts,
                        elseif_clauses: vec![],
                    ...
                    if std::env::var("LUAU_OOR_TRACE").is_ok() {
                        eprintln!(
                            "OOR pc={} op={:?} target={} end={} in_loop={} loop_end={:?}",
                            pc, op, target, end, in_loop, ctx.current_loop_end
                        );
                    }
                    if in_loop {
            ...
            | LuauOpcode::JumpIfLT | LuauOpcode::JumpIfNotLT => {
                ...
                if target > pc && target <= end && !exits_loop {
                    ...
                    // reads the stale pre-branch literal. Split arms (round 4):
                    // see the JumpIf arm.
                    let construct_end = else_range.map_or(target, |(_, ee)| ee);
                    // Rotation 126: the two-suspect window from r125. Name before and
                    // Nil after implicates premateralization; Nil already before puts
                    ...
                    if std::env::var("LUAU_OOR_TRACE").is_ok() {
                        eprintln!(
                            "OOR-XEQK pc={} op={:?} target={} end={} in_loop={} loop_end={:?}",
                            pc, op, target, end, in_loop, ctx.current_loop_end
                        );
                    }
                    if in_loop {
            ...
            | LuauOpcode::JumpXEqKN | LuauOpcode::JumpXEqKS => {
                ...
                let condition = Expr::BinOp { left: Box::new(left.clone()), op: cmp, right: Box::new(right.clone()) };
                let mut target = (pc as i32 + d as i32 + 1) as usize;
                // A branch inside this arm targeting past it is the construct's
                // own continuation; cutting here strands it into a bare
                // `continue`. See `extended_arm_end`.
                {
                    let arm_start = pc + 1 + if op.has_aux() { 1 } else { 0 };
                    target = extended_arm_end(code, arm_start, target, end);
                }
                ...
                if target > pc && target <= end && !exits_loop {
                    ...
                    // below annihilates the branch's result and the join
                    // silently reads the stale pre-branch literal. Mirrors the
                    // JumpIfEq-family arm. Split arms (round 4): see the
                    // JumpIf arm.
                    let construct_end = else_range.map_or(target, |(_, ee)| ee);
                    // Rotation 126: the two-suspect window from r125. Name before and
                    // Nil after implicates premateralization; Nil already before puts
            ...
            LuauOpcode::GetVarargs => {
                ...
                // one corpus module alone 570 times, and that single class was 40 of
                ...
                if !proto.is_vararg {
                    regs[a] = RegVal::Unknown;
                    crate::decompiler::mint_trace::note_unknown("UNK_VARARG_ILLEGAL", a);
                    pc += 1;
                    continue;
                }
            ...
            // SHL: 11 corpus hits (all in corpus modules) — all nonsensical.
            // SHR: 18 corpus hits (CFrame/SetVolume operands) — all nonsensical.
            ...
            LuauOpcode::RbxExt93 | LuauOpcode::RbxExt94 | LuauOpcode::RbxExt98 => {
                ...
                //       unrelated BANDK+LOADK pairs in a corpus module (56 hits).
            ...
            // Evidence: a corpus module's Fire(self, ...) disassembly shows
            ...
            LuauOpcode::RbxExt97 => {
                ...
                if !proto.is_vararg {
                    regs[a] = RegVal::Unknown;
                    crate::decompiler::mint_trace::note_unknown("UNK_RBXEXT97_ILLEGAL", a);
                    pc += 1;
                    continue;
                }
            ...
            LuauOpcode::RbxExt92 => {
                ...
                //   a corpus module:     self = __rbx_unary92(self) → passthrough
                //   a corpus module:     __rbx_unary92(#self.rects) → wraps length
                //   a corpus module (×2): __rbx_unary92("game") → wraps string
                //   two corpus modules:  __rbx_unary92(v0) → wraps register
            ...
            // Evidence: a corpus module (stack=4, C=8 > stack → constant index,
            // K8=0) produces `padding.top or 0`. Another corpus module (stack=9,
            // C=14 > stack, K14=0) produces `tonumber(...) or 0`. Both confirmed
            // via a live client's own bytecode disassembly.
            ...
            // Evidence:
            //   a corpus module: __rbx_binary100(Seed2, v0.Difficulty.Exponent)
            //     — field named "Exponent" → Seed ^ Exponent.
            //   a corpus module: __rbx_binary100(v3, arg3.Power)
            //     — field named "Power" → base ^ power.
            //   a corpus module: __rbx_binary100(arg1, arg4)
            //     — curve evaluation commonly uses exponentiation.
            //   a corpus module: __rbx_binary100(v3, arg2) — camera math.
            ...
            // Evidence:
            //   a corpus module: __rbx_binary101(Head, new2):IsA("BasePart")
            //     — Head and new2, chained with method → returns Instance if both truthy.
            //   a corpus module: __rbx_binary101(true, true) → true and true = true.
            //   a corpus module: __rbx_binary101(v4, true) → v4 and true.
            //   a corpus module: __rbx_binary101(match1, match2) → first and second match.
            //   a corpus module: __rbx_binary101(self:GetAttribute(v6), arg3) → attr and arg3.
            ...
            // Evidence:
            //   a corpus module: return __rbx_binary105(now2, SPAWN_EVERY)
            //     — time remaining until next spawn = now % interval.
            //   a corpus module: return __rbx_binary105(now3, interval2)
            //     — time within current interval = now % interval.
            //   a corpus module: __rbx_binary105(UnixTimestamp - offset, Interval)
            //     — time modulo period.
            //   a corpus module: math.min(v13, __rbx_binary105(result, result2)) — mod result clamped.
            ...
            LuauOpcode::Unknown => {
                ...
                if a < regs.len() {
                    regs[a] = RegVal::Unknown;
                    crate::decompiler::mint_trace::note_unknown("UNK_OPCODE_UNKNOWN", a);
                }
