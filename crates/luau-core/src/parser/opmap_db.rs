...
impl OpmapDb {
    ...
    pub fn lookup(&self, chunk: &Chunk) -> DbLookup {
        ...
        for e in &self.entries {
            // An opcode permutation is a property of the CLIENT BUILD. The
            // bytecode version is a property of the COMPILER that emitted the
            // chunk. They are orthogonal, and one client ships both: the live
            // compiler emits v9 while the precompiled CoreScripts are v7, all
            // under one shuffle.
            //
            // An equality test here conflates them. `roblox_v9_live` takes its
            // `bytecode_version` from the build label ("live v9"), not from the
            // version of the chunks it was probed against, so equality rejected
            // it on all 1880 v7 CoreGui files and every one fell back to
            // per-file heuristic detection: 23.9% clean, 6449 defects. The same
            // entry admitted decodes them at 90.4% clean, 303 defects.
            //
            // Luau opcode numbering is append-only, so an entry measured at a
            // NEWER version is a superset of an older chunk's opcode set and is
            // safe to try. The reverse is not: an older entry against newer
            // chunks leaves the added opcodes unmapped, so that stays rejected.
            //
            // This only admits the entry as a CANDIDATE. Tier-A anchor
            // conflicts, the anchor-agreement floor and `walk_verify` all still
            // run below and reject a map that does not fit.
        ...
        // Both size floors below exist to DISCRIMINATE between candidates. When
        // exactly one measured entry has already walked the chunk cleanly there
        // is nothing left to discriminate, and refusing it does not avoid a
        // guess -- it forces one, because the chunk then falls back to a
        // detector map or to raw identity bytes.
        //
        // Measured: forcing the entry on all 1517 defect-carrying CorePackages
        // files gave 435 better and 0 worse, with the 1082 files that already
        // matched byte-identical -- which is what shows this is a no-op wherever
        // the entry applied anyway. 426 files were being refused by Gate 3 for
        // "too little evidence" alone, carrying 450 undefined_local.
        //
        // The floors stay in force whenever two or more entries survive, which
        // is the only case where a size floor can separate anything. The
        // database holds one entry today; this is written for the multi-entry
        // future rather than deleted.
        let sole_measured = fitted.len() == 1 && fitted[0].0.provenance.is_measured();

        // Gate 3: enough of the chunk to be worth being exact about.
        let present = fitted.iter().map(|(_, p)| *p).max().unwrap_or(0);
        if !sole_measured && present < DB_MIN_PRESENT_BYTES {
        ...
        let mut scored: Vec<(&DbEntry, i32, u32, u8)> = fitted
            .iter()
            .map(|(e, _)| {
                let (agree, conflict) = fp.corroboration(&e.map);
                // Version distance: an entry measured at a NEWER bytecode version
                // than the chunk belongs to a LATER client build with a DIFFERENT
                // opcode shuffle. Among entries that all walk cleanly (small chunks
                // can coincidentally satisfy a foreign shuffle), the one closest to
                // the chunk's own version is the right build. v7 CoreScripts and v9
                // live scripts share one build/shuffle, so the v9 entry (bcv 9)
                // wins for both a v7 and a v9 chunk; a v12 entry (bcv 12) never
                // wins a v9 chunk. Gate 1 already guarantees bcv >= chunk.version.
                let vdist = e.bytecode_version.saturating_sub(chunk.version);
                (*e, agree as i32 - 3 * conflict as i32, agree, vdist)
            })
            .collect();
        // Closest bytecode version first, then higher corroboration, then id.
        scored.sort_by(|a, b| {
            a.3.cmp(&b.3)
                .then(b.1.cmp(&a.1))
        ...
        let (best, best_score, best_agree, best_vdist) = scored[0];
        // Same reasoning as Gate 3: corroboration ranks rival entries, and a
        // sole measured entry that already walks cleanly has no rival to be
        // ranked against. A small chunk simply cannot corroborate much.
        if !sole_measured && best_agree < DB_MIN_CORROBORATION {
        ...
        if scored.len() > 1 {
            let (runner_up, runner_score, _, runner_vdist) = scored[1];
            // Only a tie WITHIN the same bytecode version (same client build) is a
            // genuine ambiguity. A farther-version runner-up already lost on build
            // proximity above and must not force a refusal into detector fallback.
            if runner_vdist == best_vdist
                && best_score - runner_score < DB_SCORE_MARGIN
                && runner_up.map != best.map
            {
                return DbLookup::Ambiguous {
                    candidates: scored.iter().map(|(e, _, _, _)| e.id.clone()).collect(),
                    best_score,
    ...
    pub fn lookup_by_id(&self, chunk: &Chunk, id: &str) -> anyhow::Result<DbLookup> {
        ...
        // Same asymmetry as `lookup`: opcode numbering is append-only, so an
        // entry measured at a newer bytecode version covers an older chunk's
        // opcode set. `walk_verify` below is the real gate and still runs.
        if entry.bytecode_version < chunk.version {
            anyhow::bail!(
                "entry \"{}\" was measured at bytecode v{}, older than this chunk's v{}, \
                 so opcodes added since are unmapped",
                id,
