...
//! `Lvl3` used after it. A corpus module lost a `+ 2` that way.
//!
...
//!
//!   * comparing NAME STEMS collapsed `max3`/`max6`/`max9` in a corpus module's
//!     RK4 integrator into one `max`, summing one value four times;
...
fn concat_onto_itself_is_an_accumulate() {
    // `Op = Op .. " items)"` - a corpus module
    let v = binop(name("Op"), BinOp::Concat, Expr::String(" items)".into()));
...
fn a_field_read_of_itself_is_not_an_accumulate() {
    ...
    // still needed. Accepting this destroyed a corpus module.
...
//
// so the direct guard cannot see the accumulate. A corpus module lost its pool
// name this way; another corpus module had its Add branch return a fresh local
