...
//! bytecode: a corpus module `Description` (R2 = self.Amount) and
//! another corpus module `Description` (R2 = self.Amount .. " Pair").
//!
...
//! later reader in TWO previously-clean files (one corpus module — clean bytecode —
//! and another corpus module). It is a genuine residual, but pin-only threading is
...
/// A corpus module `Description`, minimized: a parked pure BinOp seed with a
...
/// value and the arm effect vanished (the corpus module's `v2` was the region-path twin).
#[test]
