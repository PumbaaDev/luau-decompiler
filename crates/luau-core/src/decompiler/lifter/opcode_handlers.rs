...
///
/// `rc_0030 Cleanup` loses nine of ten calls to this:
///
...
/// `:IsA("BasePart")` guard in `rc_0089` to a discarded local.
fn extended_arm_end(code: &[u32], arm_start: usize, target: usize, end: usize) -> usize {
    ...
    // mis-assign - `lv_0142 Beequip` needs that and is wrecked without it,
    ...
    let out = match last {
        ...
        // range (in `rc_0030` it is the loop back-edge at 103, while the
        ...
        // is what demoted a real `:IsA("BasePart")` guard in `rc_0089`.
...
/// (538_RingPulse child "OnUpdate"): six nil-seeded locals, both diamond arms
...
fn pinned_write_consumed(
    ...
    // variables: on 551_Shop the seed "v16" collided with an upvalue and two
...
///     FIXES 22_NewClickToMove, 53_PathDisplay, 85_Badges - exactly the three
...
/// `21_ClickToMove` lost its entire auto-jump guard, `doAutoJump()` included,
...
fn restore_regs_keeping_dead_defs(regs: &mut Vec<RegVal>, snapshot: &[RegVal], pc: usize) {
    ...
    for (i, slot) in regs.iter_mut().enumerate() {
        ...
        // fold a guard away: rotation 122 traced `21_ClickToMove` keeping
    ...
    while pc < end && pc < code.len() {
        ...
        match op {
            ...
            LuauOpcode::GetUpval => {
                ...
                // bases like `(new74 + new74)`, `("Sticker")` and `(0)` on
            ...
            LuauOpcode::DupTable => {
                ...
                // in lv_0129). Materialise eagerly in that case so the field
            ...
            LuauOpcode::Call => {
                ...
                // results to Unknown. `893_Utils_Matrix3D` returned
                ...
                if nresults == 1 {
                    ...
                    if nresults == 2 {
                        ...
                        if next_is_namecall_chain {
                            ...
                            // (16_StickerPlacer: `R2 = tbl.GetLocalPlayer
                ...
                // so 861_StickerPrinterAnimator emitted `i = StickerRoll...`,
            ...
            LuauOpcode::Return => {
                ...
                // variant. `277_DragManager::OnInputBegin`:
                //
            ...
            LuauOpcode::Jump | LuauOpcode::JumpBack | LuauOpcode::JumpX => {
                ...
                if target > pc && target <= end {
                    ...
                    // `368_HoneycombTools::ConstructCell`: the span 22..99
                    ...
                    //     fixes 368_HoneycombTools; NO new findings
            ...
            LuauOpcode::JumpIf | LuauOpcode::JumpIfNot => {
                ...
                // everything after it. `rc_0047 IndexController` proto #9 loses
                ...
                if let Some(chain) = recognize_or_and_chain(code, pc) {
                    ...
                    //
                    // Measured on `54_Controls::?@74` (rotations 124-129, all
                    ...
                    //     FIXES 54_Controls (the diagnosed reproducer) and
                    //           958_Drakkarmon. NO new findings.
                    ...
                    //     `388_Sticker` collapses a flattened short-circuit into
                    //     `x and (self.Amount or 1) <= #x` (the discarded value
                    //     is now assigned), and `464_LocalFlyingEntity` gains a
                ...
                if target > pc && target <= end && !exits_loop {
                    ...
                    // 162_Badges proto 10, whose diamond sits INSIDE the outer
                    ...
                    premateralize_branch_escapes_split(
                        ...
                        // declared-never-assigned defects on the union corpus
                        // (lv_0883 v5, lv_0934 v3, rc_0037 v19, rc_0046 v11,
                        // rc_0065 v3 and v9, rc_0100 v15) - the gate declined to
                    ...
                    // `54_Controls` shows a receiver arriving as `Nil` on the way
